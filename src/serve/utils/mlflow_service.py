...
def get_latest_vehicle_counter_model_run_id(location_name, direction, stage="Production"):
    logged_model_name = f'{location_name}_{direction}_vehicle_counter_onnx_model'

    client = MlflowClient()
    latest_production_models = client.get_latest_versions(logged_model_name, stages=[stage])
